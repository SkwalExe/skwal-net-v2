...
def index(request):
    ...
    modules = os.listdir("documents/sections")
    if module not in modules:
        return HttpResponse("Module not found", status=404)

    sections = os.listdir(f"documents/sections/{module}")
    if section not in sections:
        return HttpResponse("Section not found", status=404)

    section_content = open(
        f"documents/sections/{module}/{section}", "r").read()
    sidebar_content = open(
        f"documents/sections/{module}/_sidebar.md", "r").read()
